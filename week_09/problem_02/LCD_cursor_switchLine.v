module LCD_cursor(clk, rst, LCD_E, LCD_RS, LCD_RW, LCD_DATA, line, LED_out, number_btn, control_btn);
    input clk, rst, line;
            ...
            case(state)
                ...
                WRITE: begin
                    ...
                    if (cnt >= 30) begin
                        state = DELAY_T;
                        cnt = 0;
                        if (cursor_address == 7'h16) cursor_address <= 7'h40;       // 0x15까지 스크린에 표시
                        else if (cursor_address == 7'h56) cursor_address <= 7'h00;  // 0x55까지 스크린에 표시
                        else cursor_address <= cursor_address + 1;
                    end
                    else cnt = cnt + 1;
                end
                CURSOR: begin
    ...
    // switch line (DIP2)
    always @(posedge clk or posedge line) begin
        if (line) begin
            if (cursor_address < 7'h16) cursor_address <= 7'h40;
            else if (cursor_address > 7'h39) cursor_address <= 7'h00;
            // set address
            {LCD_RS, LCD_RW, LCD_DATA} = {3'b0_0_1, cursor_address};
        end
    end
    
    always @(posedge clk or posedge rst) begin
        if (rst) {LCD_RS, LCD_RW, LCD_DATA} <= 10'b0_0_0000_0001;   // clear display
        else begin
            case(state)
                FUNCTION_SET:
                    ...
                    begin
                        if (cnt == 20) begin
                            ...
                            if (control_btn == 2'b10) begin
                                if (cursor_address == 7'h00) cursor_address <= 7'h01;
                                else if (cursor_address == 7'h40) cursor_address <= 7'h16;
                                cursor_address <= cursor_address - 1;
                            end
                            // right
                            else if (control_btn == 2'b01) begin
                                if (cursor_address == 7'h15) cursor_address <= 7'h39;
                                else if (cursor_address == 7'h55) cursor_address <= 7'h54;
                                cursor_address <= cursor_address + 1;
                            end
